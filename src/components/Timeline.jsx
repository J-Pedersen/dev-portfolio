  ...
  return (
    ...
    >
      <div
        className={`
          relative 
          ${mobile ? "space-y-10" : "border-r border-slate-300 dark:border-slate-700 mr-4 pr-6 space-y-10"}
        `}
      >
        {Object.entries(grouped).map(([year, entries]) => (
          <div key={year} className="space-y-8">
            ...
            <div
              className={`
                px-2 py-1 font-semibold text-slate-900 dark:text-slate-100
                ${mobile
                  ? "border-b border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900"
                  : "sticky top-0 bg-white/70 dark:bg-slate-900/70 backdrop-blur border-b border-slate-300 dark:border-slate-700"
                }
              `}
            >
              {year}
            ...
            {entries.map((item, index) => (
              ...
              >
                ...
                {/* DATE */}
                {item.period && (
                  <p className="text-sm text-brand dark:text-brand-soft font-medium">
                    {item.period}
                  </p>
                )}
